...
vi.mock("file-type", () => ({
    fileTypeFromFile: (...args) => mockFileTypeFromFile(...args),
    fileTypeFromBuffer: vi.fn(async (data) => {
...
function enqueueSpawnPlans(...plans) {
    for (const plan of plans) {
        enqueueSpawnPlan(plan);
    }
}
function enqueueMissingCommandLookups(count) {
    for (let index = 0; index < count; index += 1) {
        enqueueSpawnPlan({ code: 1 });
    }
}
function enqueueLibreOfficeLookup() {
    enqueueSpawnPlans({ code: 1 }, { stdout: libreOfficePath, code: 0 });
}
function enqueueImageMagickLookup() {
...
import { guessFileExtension, guessExtensionFromBuffer, findImageMagickCommand, findLibreOfficeCommand, convertOfficeDocument, convertImageToPdf, convertToPdf, getTmpDir, } from "./convertToPdf";
afterEach(() => {
    spawnPlans.length = 0;
    spawnMock.mockClear();
...
describe("test command availability", () => {
    ...
    it("accepts ImageMagick convert on non-Windows", async () => {
        ...
        enqueueSpawnPlans({ code: 1 }, { stdout: "/usr/bin/convert\n", code: 0 }, { stdout: "Version: ImageMagick 6.9.12-98 Q16\n", code: 0 });
        const result = await findImageMagickCommand();
        expect(result).toStrictEqual({
...
describe("test convertOfficeDocument", () => {
    ...
    it("conversion fails (command not found)", async () => {
        enqueueMissingCommandLookups(4);
        await expect(convertOfficeDocument("test_command.docx", "./")).rejects.toThrow("LibreOffice is not installed. Please install LibreOffice to convert office documents. On macOS: brew install --cask libreoffice, On Ubuntu: apt-get install libreoffice, On Windows: choco install libreoffice-fresh");
    });
...
describe("test convertImageToPdf", () => {
    ...
    it("conversion fails (command not found)", async () => {
        enqueueMissingCommandLookups(2);
        await expect(convertImageToPdf("test_command.png", "./")).rejects.toThrow("ImageMagick is not installed. Please install ImageMagick to convert images. On macOS: brew install imagemagick, On Ubuntu: apt-get install imagemagick, On Windows: choco install imagemagick.app");
    });
});
