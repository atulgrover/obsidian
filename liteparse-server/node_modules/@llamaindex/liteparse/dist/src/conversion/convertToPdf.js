...
function getResolvedPathFromOutput(output, useLastLine = false) {
    const lines = output
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter(Boolean);
...
function isWindowsSystemConvert(filePath) {
...
async function isImageMagickBinary(executablePath, args = []) {
    try {
...
async function resolveImageMagickCommand(command) {
 ...
 */
export async function convertOfficeDocument(filePath, outputDir, password) {
    ...
    args.push(filePath);
    await executeCommand(libreOfficeCmd, args, 120000 // 2 minutes timeout
    );
 ...
 */
export async function convertToPdf(filePath, password) {
    try {
 ...
 */
export async function guessExtensionFromBuffer(data) {
 ...
 */
export async function convertBufferToPdf(data, password) {
