...
class syntax_plugin_pdfjs extends DokuWiki_Syntax_Plugin {
    ...
    public function handle($match, $state, $pos, Doku_Handler $handler) {
        ...
        $link = $media_parts[0];
        $title = isset($media_parts[1]) ? $media_parts[1] : '';

        $idzoom_parts = explode('?disp=', $link, 2);
        $idzoom = $idzoom_parts[0];
        $display = isset($idzoom_parts[1]) ? trim($idzoom_parts[1]) : '';
        //get the display
        if($display) {
        ...
        $id = $zoom_parts[0];
        $zoom = isset($zoom_parts[1]) ? $zoom_parts[1] : '';
        if($zoom) {
